...
def type_nn(config: Dict[str, Any], x_train_shape: Tuple[int, ...], delay: int, device: Union[str, torch.device]) -> nn.Module:
    ...
    The model type and its specific hyperparameters are read from the
    configuration dictionary (`config['nn']`). The input size for the model is
    inferred from `x_train_shape` based on conventions for each model type.
